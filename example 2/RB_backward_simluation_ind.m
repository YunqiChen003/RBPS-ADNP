function G=RB_backward_simluation_ind(e,m)
% e: output of Rao-Blackwellzed particle filters;
% m: model parametes;
fprintf('RB_backward_simluation_ind\n');
tic;
% Backward simulator
...
for t = (m.ss.T-1) : (-1) : 1
   %% Backward information filter update;
    if t==m.ss.T-1
        Omega_hat=m.ss.H'/m.ss.R*m.ss.H;
    else
        Omega_hat=G.Omega{t+1}+m.ss.H'/m.ss.R*m.ss.H;
    end
    ...
    for j = 1:m.ss.Ns
        %% Backward information filter update;
            ...
            innovation=m.y(:,t+1)-mea_eq(G.x_bwd(:,j,t+1));
        if t==m.ss.T-1   
            lambda_hat_cand=m.ss.H'/m.ss.R*innovation;
        else
            lambda_hat_cand=G.lambda{t+1}(:,j)+m.ss.H'/m.ss.R*innovation;
        end
       %% Backward information filter prediction;
