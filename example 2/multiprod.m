...
%         of the P譗 matrices in A by the R譙 matrices in B. [DA1 DA2] are
%         the IDs of A; [DB1 DB2] are the IDs of B.
%
%   2) 2-D by 1-D BLOCK(S) (*)
%         C = MULTIPROD(A, B, [DA1 DA2], DB1) contains the products of the
%         P譗 matrices in A by the R-element vectors in B. The latter are
...
%         Q-element vectors in A by the R譙 matrices in B. The vectors in A
%         are considered to be 1譗 matrices. DA1 is the ID of A; [DB1 DB2]
%         are the IDs of B.
%
...
%             contained in A and B into 1譖 and P�1 matrices, respectively,
...
%             contained in A and B into P�1 and 1譗 matrices, respectively,
...
%   file #23084). Together, these functions make up the 揂RRAYLAB toolbox�.
%
...
%      1) C contains P譙 matrices along IDs MAX([DA1 DA2], [DB1 DB2]).
%      2) Array     Block size     ID(s)
%         ----------------------------------------------------
%         A         P譗  (2-D)     [DA1 DA2]
%         B         R    (1-D)     DB1
%         C (a)     P    (1-D)     MAX(DA1, DB1)
%         C (b)     P譗  (2-D)     MAX([DA1 DA2], [DB1 DB1+1])
%         ----------------------------------------------------
...
%         C (b)     R譙  (2-D)     MAX([DA1 DA1+1], [DB1 DB2])
%         ----------------------------------------------------
...
%         (c) A         P        (1-D)     DA1
%             B         Q        (1-D)     DB1
%             C         P譗      (2-D)     MAX([DA1 DA1+1], [DB1 DB1+1])
%         --------------------------------------------------------------
%
%   Terminological notes:
%   (*) 1-D and 2-D blocks are generically referred to as "vectors" and 
%       "matrices", respectively. However, both may be also called
%       搒calars� if they have a single element. Moreover, matrices with a
%       single row or column (e.g. 1�3 or 3�1) may be also called 搑ow
%       vectors� or 揷olumn vectors�.
