function e=Conditional_RTS_smoothing_asyn_cor2(G,m)
% G: output of Rao-blackwellized backward simluation for nonlinear sub-state;
% m: model parameters;
fprintf('Conditional_RTS_smoothing_asyn_cor2\n');
tic;
...
for t=1:m.ss.T
    if t==1
       ...
       M=m.ss.Hbar1*Plfilterstar(:,:,t-1)*m.ss.Hbar1'+m.ss.Hbar2*m.ss.Albar*Plfilterstar(:,:,t-1)*m.ss.Hbar1'+...
            m.ss.Hbar1*Plfilterstar(:,:,t-1)*m.ss.Albar'*m.ss.Hbar2'+m.ss.Hbar2*Plpre*m.ss.Hbar2'+m.ss.R0;%PF measurement update;
       K=(m.ss.Albar*Plfilterstar(:,:,t-1)*m.ss.Hbar1'+Plpre*m.ss.Hbar2')/M;%KF measurement update;
       ...
       muy=mea_eq(G.x_bwd(:,:,t))+m.ss.gamany'*(G.x_bwd(:,:,t)-G.x_bwd(:,:,t-1))-m.ss.gamalybar'*flbar+...
           m.ss.Hbar1*Xlfilterstar(:,:,t-1)+m.ss.Hbar2*Xlpre(:,:,t);
