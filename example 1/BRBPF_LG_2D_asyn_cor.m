function e=BRBPF_LG_2D_asyn_cor(m)
fprintf('BRBPF_LG_2D_asyn_cor\n');
...
for t=1:m.ss.T
    if t==1
        ...
        W(:,t)=ones(m.ss.N,1)./m.ss.N;
         ...
         %% PF time update
        e.Sigma_nn{t}=m.ss.A(1,2)*Plfilter(:,:,t-1)*m.ss.A(1,2)'+m.ss.Qnn;
        e.alpha{t}=m.ss.A(1,1)*xparticle(:,:,t-1)+m.ss.A(1,2)*xlfilter(:,:,t-1);
        ...
        xparticle(:,:,t)=e.alpha{t}(:,nIdx(:,t-1))+sqrt(e.Sigma_nn{t})*randn(1,m.ss.N);
        %% KF time update
        % KF dynamical measurement update;
        L=Plfilter(:,:,t-1)*m.ss.A(1,2)'/e.Sigma_nn{t};    
        Xlfilterstar=xlfilter(:,nIdx(:,t-1),t-1)+L*(xparticle(:,:,t)-e.alpha{t}(:,nIdx(:,t-1)));
        Plfilterstar=Plfilter(:,:,t-1)-L*e.Sigma_nn{t}*L';
        % KF time update;
        flbar=m.ss.gamaln*(xparticle(:,:,t)-m.ss.A(1,1)*xparticle(:,nIdx(:,t-1),t-1));
       ...
       muy=m.ss.H(1,1)*xparticle(:,:,t)+m.ss.gamany'*(xparticle(:,:,t)-m.ss.A(1,1)*xparticle(:,nIdx(:,t-1),t-1))-m.ss.gamalybar'*flbar+...
           m.ss.Hbar*[Xlfilterstar;Xlpre];
       innovation=m.y(:,t)-muy;
       xlfilter(:,:,t)=Xlpre+K*(innovation);
        %% PF measurement update
        logly=loggausspdf2(innovation,inv(M))';
