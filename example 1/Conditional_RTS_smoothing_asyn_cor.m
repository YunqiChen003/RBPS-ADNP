function e=Conditional_RTS_smoothing_asyn_cor(G,m)
% G: output of Rao-blackwellized backward simluation for nonlinear sub-state;
% m: model parameters;
fprintf('Conditional_RTS_smoothing_asyn_cor\n');
tic;
...
for t=1:m.ss.T
    if t==1
        ...
        Pxx=Plfilterstar(:,:,t-1)*m.ss.Albar';
        Plpre=m.ss.Albar*Pxx+m.ss.Qllbar;
        %% KF measurement update
       Skesai=[Plfilterstar(:,:,t-1),Pxx;Pxx',Plpre];%PF measurement update;
       ...
       Minv=inv(M);
       K=Skesai(2,:)*m.ss.Hbar'*Minv;%KF measurement update;
       Sigmaly=K*M;
       Sigmalyl=Sigmaly*K';
       Plfilter(:,:,t)=Plpre-Sigmalyl;
       Plfinv=inv(Plfilter(:,:,t));
       J1{t-1}=(Pxx-Sigmalyl)*Plfinv;
       Sigmaly2=Plfilterstar(:,:,t-1)*m.ss.Hbar1'+Pxx*Plfilterstar(:,:,t-1);
       J2{t-1}=(-Pxx+Sigmaly2*K')*Plfinv*K+Sigmaly2*Minv;
       Delta{t-1}=J1{t-1}*Pxx'+J2{t-1}*Sigmaly2;
